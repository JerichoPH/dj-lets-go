...
func (receiver FileSystem) ReadString() string {
	...
	if err != nil {
		wrongs.PanicForbidden("文件读取失败:", err.Error())
	}
	return string(f)
}
...
func (receiver FileSystem) ReadJson() interface{} {
	...
	if err != nil {
		wrongs.PanicForbidden("JSON解析失败: %s", err.Error())
	}
...
func (receiver FileSystem) WriteString(value string) FileSystem {
	...
	if err != nil {
		wrongs.PanicForbidden("文件打开失败: %s", err.Error())
	}
	//及时关闭file句柄
	defer func(file *os.File) {
		err = file.Close()
		if err != nil {
			wrongs.PanicForbidden("文件关闭失败: %s", err.Error())
		}
	}(file)
	...
	if err != nil {
		wrongs.PanicForbidden("文件写入失败: %s", err.Error())
	}
