...
func (AccountRouter) Load(engine *gin.Engine) {
	...
	{
		// 新建
		r.POST("account/store", func(ctx *gin.Context) { controllers.NewAccountController().Store(ctx) })

		// 删除
		r.DELETE("account/:uuid", func(ctx *gin.Context) { controllers.NewAccountController().Delete(ctx) })

		// 编辑
		r.PUT("account/:uuid/update", func(ctx *gin.Context) { controllers.NewAccountController().Update(ctx) })
